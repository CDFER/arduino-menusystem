...
BackMenuItem::BackMenuItem(const char* name, const char* icon, SelectFnPtr select_fn,
						   MenuSystem* ms)
	: MenuItem(name, icon, select_fn),
	  _menu_system(ms) {
}
...
NumericMenuItem::NumericMenuItem(const char* basename, const char* icon, SelectFnPtr select_fn,
								 float value, float min_value, float max_value,
								 float increment,
								 FormatValueFnPtr format_value_fn)
	: MenuItem(basename, icon, select_fn),
	  _value(value),
